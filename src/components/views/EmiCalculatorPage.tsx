    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-6 bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-bold">
              <span className="text-slate-700 uppercase tracking-wider">Loan Principal Amount (P)</span>
              <span className="text-indigo-700 font-black text-sm">{formatIndianCurrency(principal)}</span>
            </div>
            <input
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-bold">
              <span className="text-slate-700 uppercase tracking-wider">Concessional Annual Interest Rate (r)</span>
              <span className="text-emerald-700 font-black text-sm">{annualRate}% p.a.</span>
            </div>
            <input
              id="slider-emi-rate"
              type="range"
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-bold">
              <span className="text-slate-700 uppercase tracking-wider">Repayment Period (Tenure)</span>
              <span className="text-indigo-700 font-black text-sm">{tenureYears} Years ({tenureYears * 12} Months)</span>
            </div>
            <input
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center text-xs font-bold">
              <span className="text-slate-700 uppercase tracking-wider">Moratorium / Grace Period</span>
              <span className="text-amber-700 font-black text-sm">{moratoriumMonths} Months</span>
            </div>
            <input
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="bg-gradient-to-br from-indigo-900 to-slate-900 text-white p-6 sm:p-8 rounded-2xl shadow-md space-y-6">
            <div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {emiResult.yearlySummary.map((row) => (
                    <tr key={row.year} className="hover:bg-slate-50">
                      <td className="py-2 font-bold text-slate-700">Year {row.year}</td>
                      <td className="py-2 text-slate-900">{formatIndianCurrency(row.principalPaid)}</td>
                      <td className="py-2 text-emerald-700 font-medium">{formatIndianCurrency(row.interestPaid)}</td>
                      <td className="py-2 text-right font-mono font-bold text-slate-800">{formatIndianCurrency(row.endingBalance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
